...
Page({
  ...
  // 加载课程类型列表
  listCourseType: function () {
    var self = this
    var url = config.basePath + "api/courseType/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          courseTypes: self.data.courseTypes.concat(res.data.list),
  ...
  //删除课程类型记录
  removeCourseType: function (e) {
    var self = this
    var courseTypeId = e.currentTarget.dataset.coursetypeid
    wx.showModal({
      title: '提示',
      content: '确定要删除courseTypeId=' + courseTypeId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/courseType/delete/" + courseTypeId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除课程类型后客户端同步删除数据
            var courseTypes = self.data.courseTypes;
            for (var i = 0; i < courseTypes.length; i++) {
              if (courseTypes[i].courseTypeId == courseTypeId) {
                courseTypes.splice(i, 1)
                break
              }
            }
            self.setData({ courseTypes: courseTypes })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
