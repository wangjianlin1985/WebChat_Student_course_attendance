...
Page({
  ...
  // 加载时段列表
  listTimeInterval: function () {
    var self = this
    var url = config.basePath + "api/timeInterval/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          timeIntervals: self.data.timeIntervals.concat(res.data.list),
  ...
  //删除时段记录
  removeTimeInterval: function (e) {
    var self = this
    var intervalId = e.currentTarget.dataset.intervalid
    wx.showModal({
      title: '提示',
      content: '确定要删除intervalId=' + intervalId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/timeInterval/delete/" + intervalId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除时段后客户端同步删除数据
            var timeIntervals = self.data.timeIntervals;
            for (var i = 0; i < timeIntervals.length; i++) {
              if (timeIntervals[i].intervalId == intervalId) {
                timeIntervals.splice(i, 1)
                break
              }
            }
            self.setData({ timeIntervals: timeIntervals })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
